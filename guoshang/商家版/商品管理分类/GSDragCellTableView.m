...
typedef enum{
    GSSnapshotMeetsEdgeTop,
    GSSnapshotMeetsEdgeBottom,
}GSSnapshotMeetsEdge;

@interface GSDragCellTableView ()
/**对被选中的cell的截图*/
...
/**自动滚动的方向*/
@property (nonatomic, assign) GSSnapshotMeetsEdge autoScrollDirection;
@property (nonatomic, strong) UILongPressGestureRecognizer *longPress;
@end

#define COLOR(R, G, B, A) [UIColor colorWithRed:R/255.0 green:G/255.0 blue:B/255.0 alpha:A]
@implementation GSDragCellTableView
...
- (void)cellSelectedAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //改变被选中的cell 的背景色
    cell.backgroundColor = COLOR(253, 134, 9, 1);
...
- (void)didEndDraging{
    ...
    if ([self.delegate respondsToSelector:@selector(cellDidEndMovingInTableView:MoveAtIndexPath:)]) {
        [self.delegate cellDidEndMovingInTableView:self MoveAtIndexPath:_relocatedIndexPath];
    }
...
- (BOOL)checkIfSnapshotMeetsEdge{
    ...
    if (minY < self.contentOffset.y) {
        _autoScrollDirection = GSSnapshotMeetsEdgeTop;
        return YES;
    }
    if (maxY > self.bounds.size.height + self.contentOffset.y) {
        _autoScrollDirection = GSSnapshotMeetsEdgeBottom;
        return YES;
    }
    return NO;
}
...
- (void)startAutoScroll{
    ...
    if (_autoScrollDirection == GSSnapshotMeetsEdgeTop) {//向下滚动

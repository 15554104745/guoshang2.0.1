...
+ (instancetype)showHUDWithCustomAnimationAddedTo:(UIView *)view {
    if (!view) {
    ...
    for (int i = 1; i <= 8; i ++) {
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"app_refresh_people_%zi",i]];
...
- (void)showAnimated:(BOOL)animated {
    MBMainThreadAssert();
    if (self.customView && [self.customView isKindOfClass:[UIImageView class]]) {
        UIImageView *imageView = (UIImageView *)self.customView;

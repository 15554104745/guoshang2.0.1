...
#import "STNoAuthorityViewController.h"
#import "STPhotoBrowser.h"
#import "ToastUtils.h"
#import <objc/runtime.h>
//导航栏主题色  
#define kNavigationBarBackColor kRGB(162, 162, 162)
double const ScalePhotoWidth = 1000;

typedef void (^handler)(NSArray<UIImage *> *selectPhotos, NSArray<STSelectPhotoModel *> *selectPhotoModels);

@interface STPhotoActionSheet () <UICollectionViewDelegateFlowLayout, UIImagePickerControllerDelegate, UINavigationControllerDelegate, PHPhotoLibraryChangeObserver, UIApplicationDelegate>
...
@implementation STPhotoActionSheet
...
- (instancetype)init
{
    self = [[kSTPhotoBrowserBundle loadNibNamed:@"STPhotoActionSheet" owner:self options:nil] lastObject];
    if (self) {
...
- (void)showWithSender:(UIViewController *)sender animate:(BOOL)animate lastSelectPhotoModels:(NSArray<STSelectPhotoModel *> *)lastSelectPhotoModels completion:(void (^)(NSArray<UIImage *> * _Nonnull, NSArray<STSelectPhotoModel *> * _Nonnull))completion
{
    [self showPreviewPhotoWithSender:sender animate:animate lastSelectPhotoModels:lastSelectPhotoModels completion:completion];
}

- (void)showPreviewPhotoWithSender:(UIViewController *)sender animate:(BOOL)animate lastSelectPhotoModels:(NSArray<STSelectPhotoModel *> *)lastSelectPhotoModels completion:(void (^)(NSArray<UIImage *> * _Nonnull, NSArray<STSelectPhotoModel *> * _Nonnull))completion
{
    [self showPreview:YES sender:sender animate:animate lastSelectPhotoModels:lastSelectPhotoModels completion:completion];
}

- (void)showPhotoLibraryWithSender:(UIViewController *)sender lastSelectPhotoModels:(NSArray<STSelectPhotoModel *> *)lastSelectPhotoModels completion:(void (^)(NSArray<UIImage *> * _Nonnull, NSArray<STSelectPhotoModel *> * _Nonnull))completion
{
    [self showPreview:NO sender:sender animate:NO lastSelectPhotoModels:lastSelectPhotoModels completion:completion];
}

- (void)showPreview:(BOOL)preview sender:(UIViewController *)sender animate:(BOOL)animate lastSelectPhotoModels:(NSArray<STSelectPhotoModel *> *)lastSelectPhotoModels completion:(void (^)(NSArray<UIImage *> * _Nonnull, NSArray<STSelectPhotoModel *> * _Nonnull))completion
{
...
- (void)loadPhotoFromAlbum
{
    ...
    [self.arrayDataSources addObjectsFromArray:[[STPhotoTool sharePhotoTool] getAllAssetInPhotoAblumWithAscending:NO]];
...
- (IBAction)btnPhotoLibrary_Click:(id)sender
{
    if (![self judgeIsHavePhotoAblumAuthority]) {
        //无相册访问权限
        STNoAuthorityViewController *nvc = [[STNoAuthorityViewController alloc] initWithNibName:@"STNoAuthorityViewController" bundle:kSTPhotoBrowserBundle];
        [self presentVC:nvc];
    } else {
        self.animate = NO;
        
        STPhotoBrowser *photoBrowser = [[STPhotoBrowser alloc] initWithStyle:UITableViewStylePlain];
        ...
        [photoBrowser setDoneBlock:^(NSArray<STSelectPhotoModel *> *selPhotoModels, BOOL isSelectOriginalPhoto) {
...
{
    STProgressHUD *hud = [[STProgressHUD alloc] init];
    [hud show];
    ...
    for (int i = 0; i < self.arraySelectPhotos.count; i++) {
        STSelectPhotoModel *model = self.arraySelectPhotos[i];
        [[STPhotoTool sharePhotoTool] requestImageForAsset:model.asset scale:scale resizeMode:PHImageRequestOptionsResizeModeExact completion:^(UIImage *image) {
            strongify(weakSelf);
...
- (void)presentVC:(UIViewController *)vc
{
    ...
    [nav.navigationBar setBackgroundImage:[self imageWithColor:kNavigationBarBackColor] forBarMetrics:UIBarMetricsDefault];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info
{
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        if (strongSelf.handler) {
            ...
            [[STPhotoTool sharePhotoTool] saveImageToAblum:image completion:^(BOOL suc, PHAsset *asset) {
...
- (void)getImageWithAsset:(PHAsset *)asset completion:(void (^)(UIImage *image, NSDictionary *info))completion
{
    ...
    size.width  *= 1.5;
    size.height *= 1.5;
    [[STPhotoTool sharePhotoTool] requestImageForAsset:asset size:size resizeMode:PHImageRequestOptionsResizeModeFast completion:completion];
}

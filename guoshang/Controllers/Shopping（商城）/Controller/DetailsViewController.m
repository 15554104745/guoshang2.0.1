...
-(void)createUI{
    ...
    _scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, Width, Height-60)];
    ...
    detailButton.frame = CGRectMake(0, 20, Width/2, 40);
    detailButton.tag = 10;
    ...
    [_scrollView addSubview:parameterButton];
    
    recommendButton = [UIButton buttonWithType:UIButtonTypeCustom];
    recommendButton.frame = CGRectMake(Width/2, 20, Width/2, 40);
...
//    _detailView.showsVerticalScrollIndicator = NO;
    ...
    //1.创建webview
    detailWebView = [[UIWebView alloc]initWithFrame:self.view.bounds];
    //根据屏幕大小自动调整页面尺寸
    detailWebView.scalesPageToFit = YES;
    detailWebView.scrollView.delegate = self;
    [_detailView addSubview:detailWebView];
    //2.设置请求URL
...
-(void)createRecommendView{
    ...
    for (int i=0; i<6; i++) {
        ...
        button.tag =16+i;
        [button setImage:[UIImage imageNamed:@" ic_load_image_pleaceholder"] forState:UIControlStateNormal];
//        [button setImageForState:UIControlStateNormal withURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",[_recommendArray[i] goods_img]]]];
        [button setImageForState:UIControlStateNormal withURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",[_recommendArray[i] goods_img]]] placeholderImage:[UIImage imageNamed:@"ic_load_image_pleaceholder"]];
//        NSLog(@"%@",[_recommendArray[i] goods_img]);
...
////
////    
//    if (scrollView == detailWebView.scrollView && scrollView.contentOffset.y < 0) {
//        detailWebView.userInteractionEnabled = NO;

...
@interface GSSearchViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate>
{
    UIView * searchView;
...
- (void)viewDidLoad {
    ...
    self.navigationController.navigationBar.barTintColor = GSNavColor;
    //将搜索历史保存到本地
    NSArray * paths=NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask,YES);
    NSString * path=[paths objectAtIndex:0];
    //在path路径下创建文件
    fileName =[path stringByAppendingPathComponent:@"GSBusinessSearchHistory.plist"];
...
-(void)search{
    if (_searchBar.text.length) {
        ...
        GSSearchResultViewController * srvc = [[GSSearchResultViewController alloc]init];
//        srvc.urlStr = URLDependByBaseURL(@"/Api/Repository/GoodsList");// Repository/GoodsList  ?m=Api&c=Category&a=category
        srvc.words = _searchBar.text;
//        srvc.params = @{@"keyword":_searchBar.text, @"page":@"1"};
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    GSSearchResultViewController * srvc = [[GSSearchResultViewController alloc]init];
    srvc.words = dataArray[indexPath.row];

//    srvc.params = @{@"keyword":dataArray[indexPath.row], @"page":@"1"};

//    srvc.urlStr = URLDependByBaseURL(@"/Api/Repository/GoodsList");
//    srvc.params = @{@"keyword":dataArray[indexPath.row], @"page":@"1"};
...
-(UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    ...
    deleteBtn.backgroundColor =GSNavColor;

...
-(void)addFounctionButton
{
    ...
    _ButtonscrollView.contentSize = CGSizeMake(Width + 120,30);
    ...
    _Rdetail.tag = 12;
    [_Rdetail setTitle:@"购买记录" forState:UIControlStateNormal];
    ...
    _BuyRecord.tag = 13;
    [_BuyRecord setTitle:@"充值卡明细" forState:UIControlStateNormal];
...
-(void)Sure:(UIButton *)button
{
    ...
    NSString *  encryptString;
    NSString * userId = [NSString stringWithFormat:@"user_id=%@,amount=%@",[[NSUserDefaults standardUserDefaults]objectForKey:@"enter"],[NSString stringWithFormat:@"%@",model.amount_org]];
    encryptString = [userId encryptStringWithKey:KEY];
    ...
    [HttpTool POST:URLDependByBaseURL(@"/Api/User/get_rechargeCard_info") parameters:@{@"token":encryptString} success:^(id responseObject) {
        ...
        Money.ali_pay_order_sn = responseObject[@"result"][@"ali_pay_order_sn"];
        Money.notify_url = responseObject[@"result"][@"notify_url"];
        Money.pay_order_sn = responseObject[@"result"][@"pay_order_sn"];
...
-(void)loadRecordData
{
    ...
    [HttpTool POST:URLDependByBaseURL(@"/Api/User/ChargeCardDetails") parameters:@{@"token":encryptString} success:^(id responseObject) {
        ...
        for (NSDictionary *dict in responseObject[@"result"]) {
           ...
           [_backView addSubview:self.tabelView];
        [self.tabelView reloadData];

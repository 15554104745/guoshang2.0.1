...
- (NSString *)encryptStringWithKey:(NSString*)_encryptKey
{
    ...
    _encryptKey = (_encryptKey == nil)? [self get24BitKey:@"51kuaiting51kuaiting51kuaiting"]: [self get24BitKey:_encryptKey];
...
{
    if (keyString.length == 30) return keyString;
    
    if (keyString.length > 30){
        keyString = [keyString substringWithRange:(NSRange){0,24}];
    }
    else{
        keyString = [keyString stringByAppendingFormat:[NSString stringWithFormat:@"%%0%lud",30 - keyString.length], 0];
    }
    return keyString;
}

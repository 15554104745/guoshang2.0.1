...
-(void)setModel:(GSGroupInfoModel *)model{
    ...
    self.wareLine.image = [UIImage imageNamed:@"xuxian"];
     ...
     self.flagImg.image = [UIImage imageNamed:_model.status];
    [self.imgView sd_setImageWithURL:[NSURL URLWithString:model.goods_info[@"goods_img"]] placeholderImage:[UIImage imageNamed:@"ic_load_image_pleaceholder"]];
    self.titleLabel.text = [NSString stringWithFormat:@"%@ %@",model.title,model.descrip];
//    self.descripLabel.text = model.descrip;
    self.peopleLabel.text = [NSString stringWithFormat:@"%@人团",model.user_num];
    self.priceLabel.text = [NSString stringWithFormat:@"￥%@",model.group_price];

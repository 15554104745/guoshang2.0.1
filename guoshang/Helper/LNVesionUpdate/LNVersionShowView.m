...
#define WIDTH   Width * 580/2/375
#define HEIGHT Height * 627/2/667
#define SPACE  Height * 50/667
#define TITLEFONT 24*Height/667
#define VERSIONFONT 15*Height/667
#define WIDTHSPACE Width * 15/375
#define UIColorFromRGB(rgbValue) [UIColor colorWithRed:((float)((rgbValue & 0xFF0000) >> 16))/255.0 green:((float)((rgbValue & 0xFF00) >> 8))/255.0 blue:((float)(rgbValue & 0xFF))/255.0 alpha:1.0]
#define NavigationBarColor UIColorFromRGB(0x60689f)
#define WindowView      [[[UIApplication sharedApplication] delegate] window]
#define SYSTEMVERSION [[[UIDevice currentDevice] systemVersion] floatValue]
@interface LNVersionShowView()<UITableViewDelegate,UITableViewDataSource>
/**
 *  标题
 */
...
@implementation LNVersionShowView
...
(void)show
{
    ...
    self.frame = CGRectMake((Width- WIDTH)/2, (Height - HEIGHT)/2 - SPACE, WIDTH, HEIGHT);
    }];
...
- (void)removeView
{
    [UIView animateWithDuration:0.25 animations:^{
        ...
        [self setFrame:CGRectMake((Width - WIDTH)/2, Height, WIDTH, HEIGHT)];
        ...
        if (self.removeShowViewBlock) {
            
            self.removeShowViewBlock();
         }
    }];
...
//   NSLog(@"最新的version%@",self.vesionStr);
    
    [[NSUserDefaults standardUserDefaults] setObject:self.vesionStr forKey:@"VersionUpdateNotice"];
...
-(NSString *)getAppStroeUrlForiPhone{
    NSDictionary * dic = [self.infocontent objectAtIndex:0];
//    NSLog(@"%@",dic);
    NSString * str = [dic objectForKey:@"trackViewUrl"];
//    NSLog(@"*****%@",str);

...
-(void)buttonclick:(UIButton *)button{
    ...
    if (button.tag == 10) {
        NSLog(@"&&&&%d",_numTF.text.length);
        
        if (_numTF.text.length == 11) {
            NSString  *encryptString;
            
            NSString * account = [NSString stringWithFormat:@"mobile=%@,password=%@",_numTF.text,_pwdTF.text];
            
            encryptString = [account encryptStringWithKey:KEY];
            NSLog(@"%@",encryptString);
            
            [HttpTool POST:@"http://www.ibg100.com/Apiss/index.php?m=Api&c=User&a=login" parameters:@{@"token":encryptString} success:^(id responseObject) {
        ...
        snsPlatform.loginClickHandler(self,[UMSocialControllerService defaultControllerService],YES,^(UMSocialResponseEntity *response){
            ...
            if (response.responseCode == UMSResponseCodeSuccess) {
                ...
                NSLog(@"%@",encryptString);
                [HttpTool POST:@"http://www.ibg100.com/Apiss/index.php?m=Api&c=User&a=third_login" parameters:@{@"token":encryptString} success:^(id responseObject) {
                    NSLog(@"%@",responseObject);

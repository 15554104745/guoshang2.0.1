//
//  GSCunstomAllOrderViewController.m
//  guoshang
//
...
@interface GSCustomAllOrderViewController ()
...
@implementation GSCustomAllOrderViewController
...
-(void)createCustomData{
    ...
    NSString * userId = [NSString stringWithFormat:@"shop_id=%@,page=%d,goods_type=%@,order_sn=%@",self.shop_id,self.page,self.goods_type,self.order_number];
    encryptString = [userId encryptStringWithKey:KEY];
    NSLog(@"%@",GS_Business_Shop_id);
...
-(void)createUserData{
    ...
    NSString * userId = [NSString stringWithFormat:@"shop_id=%@,page=%d", self.shop_id,self.page];
    encryptString = [userId encryptStringWithKey:KEY];
//    NSLog(@"%@",encryptString);
    __weak typeof(self) weakSelf = self;

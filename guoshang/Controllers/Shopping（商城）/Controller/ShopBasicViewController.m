...
@interface ShopBasicViewController ()<UITableViewDelegate,UITableViewDataSource,UICollectionViewDelegateFlowLayout>
{
//    BOOL isCome;
    BOOL isChanged;
    BOOL isSifted;
...
@implementation ShopBasicViewController
...
//    isCome = NO;
    ...
    _url     = URLDependByBaseURL(@"?m=Api&c=Category&a=category");
...
//界面创建
-(void)createUI{
    UIBarButtonItem * backItem = [UIBarButtonItem barButtonItemWithBackgroundImage:[UIImage imageNamed:@"fanhui"] highlightedImage:nil target:self action:@selector(backToHome:) forControlEvents:
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([tableView isEqual:_tableView]) {
        ...
        gsvc.recommendModel = _dataArray[indexPath.row];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([[_dataArray[0] is_exchange] isEqualToNumber:@0]) {
        ...
        //判断是否是促销商品
        if (![[_dataArray[indexPath.row] promote_price] isEqualToString:@""]) {
            cell.priceLabel.text = [_dataArray[indexPath.row] promote_price];
        }
    }else if ([[_dataArray[0] is_exchange] isEqualToNumber:@1]){
        //国币

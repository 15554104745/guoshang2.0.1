...
#define GSWeakSelf __weak typeof(self) weakSelf = self

@interface GoodsShowViewController ()<UIScrollViewDelegate,ImagesScrollViewDelegate>

{
    ...
    NSInteger is_toBuyNew; //从立即购买进入选择商品界面
    
    
}
@property (nonatomic , copy)NSMutableString *attr_id;
@end
...
-(void)dataInit{
    ...
    if (_goodId) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            NSString * str = [NSString stringWithFormat:@"goods_id=%@",_goodId];
            NSString * token  = [str encryptStringWithKey:KEY];
            dispatch_group_enter(getDataGroup);
            [HttpTool POST:URLDependByBaseURL(@"/Api/Repository/GoodsDetail") parameters:@{@"token":token} success:^(id responseObject) {
...
//                    }
                    ...
                    for (NSDictionary * dic1 in responseObject[@"result"][@"pictures"]) {
                        [_picArray addObject:dic1[@"img_original"]];
                    }
...
-(void)createUI{
    ...
    backButton.tag = 21;
    [backButton addTarget:self action:@selector(otherGoodsButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    cartButton.tag = 22;
    [cartButton addTarget:self action:@selector(otherGoodsButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    menuButton.tag = 23;
    [menuButton addTarget:self action:@selector(otherGoodsButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [collectButton addTarget:self action:@selector(otherGoodsButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    addToButton.tag = 25;
    [addToButton addTarget:self action:@selector(otherGoodsButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    buyButton.tag = 26;
    [buyButton addTarget:self action:@selector(otherGoodsButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if (cartCount != 0 && [[_dataArray[0] is_exchange] isEqualToNumber:@0]) {
...
//       UIViewContentModeScaleAspectFill
    [adSC setImageViewContentMode:UIViewContentModeScaleToFill];
    ...
    titleLabel.textColor = [UIColor colorWithRed:45/255.0 green:45/255.0 blue:45/255.0 alpha:1];
    ...
    if (_dataArray.count > 0) {
        desLabel.frame = CGRectMake(10, titleLabel.frame.origin.y+30, Width, 40);
    ...
    if ([[_dataArray[0] is_exchange] isEqualToNumber:@0]) {
        UILabel * priceLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, Y, 90, 30)];
        ...
        if ([[_dataArray[0] is_give_integral] isEqualToNumber:@0]) {
            ...
            NSString *preLabelPrice = [NSString stringWithFormat:@"  送%@国币",[_dataArray[0] shop_price]];
//            LNLabel  *preLabel = [LNLabel addLabelWithTitle:preLabelPrice TitleColor:[UIColor whiteColor] Font:12  BackGroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"duihuakuang"]]];
//            [contentView addSubview:preLabel];
            
            UILabel *preLabel = [[UILabel alloc]initWithFrame:CGRectMake(90, Y, 130, 20)];
            preLabel.text = preLabelPrice;
...
//            CGSize size = [LNLabel calculateLableSizeWithString:preLabelPrice AndFont:12];
//            preLabel.frame = CGRectMake(90, 70, size.width + 20, size.height+5);
            [contentView addSubview:preLabel];
//            UIEdgeInsets insets =UIEdgeInsetsMake(25, 10, 25, 10);
//            UIImage *image0 = [UIImage imageNamed:@"duihuakuang"];
//            UIImage *image = [image0 resizableImageWithCapInsets:insets resizingMode:UIImageResizingModeStretch];
            preLabel.textAlignment = NSTextAlignmentCenter;
//            UIImage *image = [self imageResize:[UIImage imageNamed:@"duihuakuang"] andResizeTo:size];
        ...
        //是否支持充值卡
        if ( [_dataArray[0] is_recharge_card_pay] != nil && [[_dataArray[0] is_recharge_card_pay] integerValue] == 0) {
            UILabel  *preLabel = [[UILabel alloc]initWithFrame:CGRectMake(90, Y, 160, 20)];
            CGSize size2 = CGSizeMake(preLabel.frame.size.width, preLabel.frame.size.height+5);
            UIImage *image = [self imageResize:[UIImage imageNamed:@"duihuakuang"] andResizeTo:size2];
            preLabel.backgroundColor = [UIColor colorWithPatternImage:image];
        ...
        UIImageView * iconImage = [[UIImageView alloc]initWithFrame:CGRectMake(15, Y, 23, 26.5)];
    ...
    //原价
    UILabel * ppLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, 100, 200, 30)];
...
#pragma mark- 修改 --------------------------------------------------------------------------------------------
    ...
    selectButton.tag = 10;
    [selectButton addTarget:self action:@selector(otherGoodsButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if (![shoplogo isKindOfClass:[NSNull class]] && shoplogo.length > 0) {
        ...
        headerImgV.image = [UIImage imageNamed:@"ic_load_image_pleaceholder"];
    }
    [shopView addSubview:headerImgV];
    
    //商店名字label
    UILabel *shopNameLabel = [[UILabel alloc]init];
    NSString *shoptitle = [shopDic objectForKey:@"shop_title"];
    if (![shoptitle isKindOfClass:[NSNull class]] && shoptitle != nil) {
        shopNameLabel.text = shoptitle;
    }
    shopNameLabel.font = [UIFont systemFontOfSize:13];
    shopNameLabel.textColor = [UIColor grayColor];
    [shopView addSubview:shopNameLabel];
    
    if ([[shopDic objectForKey:@"shop_id"] integerValue] != 0) {
        ...
        [attractBtn setImage:[UIImage imageNamed:@"yiguanzhuh"] forState:UIControlStateSelected];
...
-(void)createView{
    ...
    if (attrArray.count != 0) {
        ...
        self.attr_id = [NSMutableString stringWithString:[attrArray[0] ID]];
    }
    if ([[_dataArray[0] is_exchange] isEqualToNumber:@0]) {
    ...
    if (attrArray.count != 0) {
        ...
        for (int i =0; i<attrArray.count; i++) {
            ...
            [typeButton addTarget: self action:@selector(chooseGoodsTypeButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [descButton addTarget: self action:@selector(goodsTypeClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [addButton addTarget: self action:@selector(goodsTypeClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [certainButton addTarget: self action:@selector(goodsTypeClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [cancelButton addTarget: self action:@selector(goodsTypeClick:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)otherGoodsButtonClick:(UIButton *)button{
    ...
    switch (button.tag) {
        ...
        case 25://加入购物车
            ...
            if (self.from.integerValue==1 || [[_dataArray[0] is_exchange] isEqualToNumber:@1]) {
...
-(void)addToCart{
    ...
    GSWeakSelf;
    //暂定商品属性
    if (self.attr_id.length == 0) {
        self.attr_id = [NSMutableString stringWithFormat:@"0"];
    }
    ...
    [HttpTool POST:URLDependByBaseURL(@"/Api/Cart/new_add_cart") parameters:@{@"token":[dic paramsDictionaryAddSaltString]} success:^(id responseObject) {
        ...
        if (([responseObject[@"status"] isEqual:@0])) {
            ...
            //在商品详情页点击立即购买直接进入并且商品不支持国币  商品支持国币时情况在上一个方法中
            if (is_toBuyNew == 1 && ![is_exchange isEqualToString:@"1"]) {
                [weakSelf payMoney]; //支付
            }
            
            
            if ([is_exchange isEqualToString:@"1"]) {
                gbvc.hidesBottomBarWhenPushed = YES;
                [weakSelf.navigationController pushViewController:gbvc animated:YES];
            }else{
                cartCount += goodsCount;
                badgeLabel.text = [NSString stringWithFormat:@"%ld",cartCount];
                [weakSelf.view addSubview:badgeLabel];
            }
...
//选择商品属性按钮点击事件
-(void)goodsTypeClick:(UIButton *)button{
    ...
    switch (button.tag) {
        ...
        case 3:{
            ...
            if (UserId) {
                //从选择尺码、颜色分类进入
                if (attrArray.count==0) {
                    [selectButton setTitle:[NSString stringWithFormat:@"已选 数量 *%ld",goodsCount+cartCount] forState:UIControlStateNormal];
                }else{
                    [selectButton setTitle:[NSString stringWithFormat:@"已选 %@ 数量 *%ld",type,goodsCount+cartCount] forState:UIControlStateNormal];
                }
...
//类型属性选择
-(void)chooseGoodsTypeButtonClick:(UIButton *)button{
    for (int i = 50 ; i<attrArray.count+50; i++) {
        if (button.tag == i) {
            ...
            self.attr_id = [NSMutableString stringWithString:[attrArray[i-50] ID]];
...
-(NSString *)imagesScrollView:(ImagesScrollView *)imagesScrollView imageUrlStringWithIndex:(NSInteger)index{
    if (_picArray.count > 0) {
        ...
        if ([subStr isEqualToString:@"http"]) {
            ...
            NSString *urlStr = [NSString stringWithFormat:@"%@%@",ImageBaseURL,_picArray[index]];
            return urlStr;
        }
    }else {
        ...
        if ([subStr isEqualToString:@"http"]) {
            ...
            return [NSString stringWithFormat:@"%@%@",ImageBaseURL,image_name_Url];
        }
    }
}
...
- (NSMutableString *)attr_id {
    if (!_attr_id) {
        _attr_id = [[NSMutableString alloc]init];
    }
    return _attr_id;
}

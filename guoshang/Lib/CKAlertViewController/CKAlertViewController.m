...
+ (instancetype)actionWithTitle:(NSString *)title backgroundColor:(UIColor *)backgroundColor titleColor:(UIColor *)titleColor handler:(void (^)(CKAlertAction *))handler {
    CKAlertAction *instance = [self actionWithTitle:title handler:handler];
    instance -> _backgroundColor = backgroundColor;
...
- (void)creatAllButtons {
    ...
    for (int i=0; i<self.actions.count; i++) {
        ...
        SFHighLightButton *btn = [SFHighLightButton buttonWithType:UIButtonTypeCustom];
        btn.tag = 10+i;
        btn.backgroundColor = self.actions[i].backgroundColor ? self.actions[i].backgroundColor : [UIColor whiteColor];
        
        btn.titleLabel.font = [UIFont systemFontOfSize:14];
        [btn setTitleColor:self.actions[i].titleColor ? self.actions[i].titleColor : kThemeColor forState:UIControlStateNormal];
...
- (void)updateAllSeparatorLineFrame {
    ...
    for (int i=0; i<linesAmount; i++) {
        ...
        separatorLine.frame = CGRectMake(x, y - 0.5, width, 0.5);
    }
}

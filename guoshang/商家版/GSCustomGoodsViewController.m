...
@interface GSCustomGoodsViewController ()
...
@implementation GSCustomGoodsViewController
...
//    self.shop_id = @"42";
//    [self.navigationItem.leftBarButtonItem setAction:@selector(backToMine)];
...
-(void)createCustomData{
    ...
    NSString * userId = [NSString stringWithFormat:@"shop_id=%@,page=%d,goods_type=%@,order_sn=%@,type=un_send", self.shop_id,self.page,self.goods_type,self.order_number];
...
-(void)createUserData{
    ...
    NSString * encryptString;
    NSString * userId = [NSString stringWithFormat:@"shop_id=%@,page=%d,order_state=3", self.shop_id,self.page];
    encryptString = [userId encryptStringWithKey:KEY];
    __weak typeof(self) weakSelf = self;

...
#import "GSCarMoneyViewController.h"
#import "MoneyCell.h"
#import "GoodsModel.h"
#import "TotalModel.h"
...
@interface GSCarMoneyViewController ()
{
    //    MoneyButton *selectButton;
    NSString *_shipping_id;
    UIView *buttonView;
    NSString *_totlaprice;
...
@implementation GSCarMoneyViewController
...
//            }];
            ...
            dispatch_group_enter(getDataGroup);
            [HttpTool POST:URLDependByBaseURL(@"/Api/Repository/checkOut") parameters:@{@"token":[dic paramsDictionaryAddSaltString]} success:^(id responseObject) {
                ...
                if ([responseObject[@"status"]isEqualToNumber:@3]) {
                    ...
                    NSArray *shipArr = [rootDic objectForKey:@"shipping"];
                    for (NSDictionary *dic in shipArr) {
//                        NSLog(@"%@",dic[@"shipping_name"]);
                        [weakSelf.shippingArr addObject:dic];
                    }
                    NSLog(@"%@",weakSelf.shippingArr);
                    //请求商品数据
...
//                    for (NSDictionary * dic in goodArray) {
                        ...
                        NSArray *arr = [rootDic objectForKey:@"goods_list"];
...
//                        [list setObject:[dic objectForKey:@"getShopLogo"] forKey:@"getShopLogo"];
...
-(void)settingData{
    ...
    if (_totalArray.count > 0) {
        ...
        if (self.isGB) {
            ...
            _totlaprice = [NSString stringWithFormat:@"%@",model.goods_price];
...
-(void)toConfim{
    ...
    if (self.addressStr.length > 2) {
        ...
        NSDictionary *dic = @{
                              @"shipping_id":shipping_id_str ? shipping_id_str : @"",
                              @"shop_id":shop_id_str ? shop_id_str : @"",
                              @"address_id":self.addressStr,
                              @"rec_id":self.tokenStr};
        __weak typeof(self) weakSelf = self;
        [HttpTool POST:URLDependByBaseURL(@"/Api/Repository/CreatePurchaseOrder") parameters:@{@"token":[dic paramsDictionaryAddSaltString]} success:^(id responseObject) {
...
//                myOrderPay.navigationItem.hidesBackButton = YES;
                myOrderPay.shopID = [NSString stringWithFormat:@"%@",shop_id_str];
...
//                myOrderPay.all_goods_price = [NSString stringWithFormat:@"%@", responseObject[@"result"][@"order_amount"]];
                myOrderPay.all_goods_price = _totlaprice;
                myOrderPay.all_goods_count =[NSString stringWithFormat:@"%@", responseObject[@"result"][@"goods_num"]];
                myOrderPay.isOrder = YES;
                [weakSelf.navigationController pushViewController:myOrderPay animated:YES];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    if (!self.isGB) {
        //判断是否是国商自营
        //NSLog(@"%lu",(unsigned long)self.shippingArr.count);
...
- (void)deliverStyButton:(MoneyButton *)sender {
    ...
    if (tag == section+100) {
        ...
        _totlaprice = [NSString stringWithFormat:@"%.2f",price + freight];
        ...
        [self.buttonSlected setObject:@"0" forKey:buttonTag2];
         _totlaprice = [NSString stringWithFormat:@"%.2f",price];

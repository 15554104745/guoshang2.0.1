...
@implementation STPhotoAblumList


@end


@implementation STPhotoTool

static STPhotoTool *sharePhotoTool = nil;
...
- (NSArray<STPhotoAblumList *> *)getPhotoAblumList
{
    NSMutableArray<STPhotoAblumList *> *photoAblumList = [NSMutableArray array];
    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL *stop) {
        ...
        if(collection.assetCollectionSubtype != 202 && collection.assetCollectionSubtype < 212){
            ...
            if (assets.count > 0) {
                STPhotoAblumList *ablum = [[STPhotoAblumList alloc] init];
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *photosBytes))completion
{
    ...
    for (int i = 0; i < photos.count; i++) {
        STSelectPhotoModel *model = photos[i];
